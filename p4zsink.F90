MODULE p4zsink
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   sinknar, sinknir   !: Heterotrophic bacteria sinking fluxes 
...
CONTAINS
   ...
   SUBROUTINE p4z_sink ( kt, knt )
      ...
      !CALL trc_sink( kt, wsbio3, sinknar , jpnar, rfact2 )
      !CALL trc_sink( kt, wsbio3, sinknir , jpnir, rfact2 )
     ...
     IF( iom_use( "tcexp" ) .OR. ( ln_check_mass .AND. kt == nitend .AND. knt == nrdttrc )  )  &
        &   t_oce_co2_exp = glob_sum( 'p4zsink', ( sinking(:,:,ik100) + sinking2(:,:,ik100) + sinknar(:,:,ik100) + sinknir(:,:,ik100) ) * e1e2t(:,:) * tmask(:,:,1) )
     !
     IF( lk_iomput ) THEN
       IF( knt == nrdttrc ) THEN
          ...
          IF( iom_use( "EPC100" ) )  THEN
              zw2d(:,:) = ( sinking(:,:,ik100) + sinking2(:,:,ik100) + sinknar(:,:,ik100) + sinknir(:,:,ik100) ) * zfact * tmask(:,:,1) ! Export of carbon at 100m
          ...
          IF( iom_use( "EXPC" ) )  THEN
              zw3d(:,:,:) = ( sinking(:,:,:) + sinking2(:,:,:) + sinknar(:,:,:) + sinknir(:,:,:) ) * zfact * tmask(:,:,:) ! Export of carbon in the water column
   ...
   INTEGER FUNCTION p4z_sink_alloc()
      ...
      ALLOCATE( sinking(jpi,jpj,jpk) , sinking2(jpi,jpj,jpk)                    ,     &                
         ...
         &      sinkfer(jpi,jpj,jpk), sinknar(jpi,jpj,jpk), sinknir(jpi,jpj,jpk), STAT=ierr(1) )                
         !
